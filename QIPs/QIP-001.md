...
4. **Sign and relay:** If not yet signed, sign PSBT, append own short ID, relay to other peers.
5. **Threshold:** If `len(signers_short_ids) >= 10`, any node MAY build and publish the block via standard `block` or `cmpctblock` messages.
